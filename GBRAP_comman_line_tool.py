...
class GBRAP():
    ...
    #    '''same as 'seq' but for introns because the cordinates are one+ than the normal cordinates'''
    ...
    # def cols (self,colvariable,overcol_variable):
    #     return colvariable[0],colvariable[1],colvariable[2],colvariable[3],colvariable[4],colvariable[5],colvariable[6],colvariable[7],colvariable[8],colvariable[9],colvariable[10],colvariable[11],colvariable[12],colvariable[13],colvariable[14],colvariable[15],overcol_variable[5]
    def run(self):
        gbff_file_path = open(self.input_path)        
        
        with open(self.output_path,'w') as outputfile:
            outputfile=csv.writer(outputfile)
            outputfile.writerow(['Class','Organism','Taxon','Locus_ID','Definition',
            ...
            for result in re.findall('LOCUS(.*?)//\n',gbff_file_path.read(), re.S): #takes all the info in each locus
                ...
                if any (word in definition.group(1).lower() for word in skiplocus):
                    ...
                    rRNAdicts={'lists':{'positives':[],'negatives':[],'pos_overlaps':[],'neg_overlaps':[]},
                              ...
                              'overlap_counters':{'pos_number':0,'neg_number':0,'totA':0,'totC':0,'totG':0,'totT':0,'totN':0,'totBase':0,'freqA':0,'freqC':0,'freqG':0,'freqT':0,'freqN':0,'freqGC':0,'freq_counter':0,
                                      'tot_PF_charg':0,'tot_CT_charg':0,'chargaff_counter':0,'tot_shan':0,'shan_counter':0,'tot_topo':0,'topo_counter':0}}
                    for each in re.findall(r'\s{5}gene\s{12}.*?(?=\s{5}gene\s{12}|$)',features.group(1),re.S):
                    ...
                    ##########FINAL########
                    outputfile.writerow([category,organism,taxa,col1,col2,
            ...
            print ("SKIPPED",skipped, "loci due to being either","/".join(skiplocus) )

                
# def main():
#     parser=argparse.ArgumentParser(description="This program will parse and analyze genomic genbank files (.gbff) and generate a csv with the data")
#     parser.add_argument("-in", help="gbff input file name (gbff or txt format)", dest="input", type=str, required=True)
#     parser.add_argument("-out", help="csv output filename.csv", dest="output", type=str, required=True)
...
def main():
    ...
    bp_gene_A: Number of 'A' bases in genes
    bp_gene_T: Number of 'T' bases in genes
    bp_gene_C: Number of 'C' bases in genes
    bp_gene_G: Number of 'G' bases in genes
    bp_gene_N: Number of 'N' bases in genes
    bp_gene_total: Total number of bases in genes
    ...
    GC_gene: Percentage of 'G' and 'C' bases in genes
    gene_topological_entropy: Topological entropy calculated from gene sequences
    gene_chargaff_pf: Chargaff's parity rule 2 score for genes
    gene_chargaff_ct: Chargaff's CT score for genes
    gene_shannon: Shannon entropy score for genes
    bp_gene_overlap_total: Number of bases overlapping between genes on two strands
    bp_cds_A: Number of 'A' bases in CDS (coding sequences)
    bp_cds_T: Number of 'T' bases in CDS
    bp_cds_C: Number of 'C' bases in CDS
    bp_cds_G: Number of 'G' bases in CDS
    bp_cds_N: Number of 'N' bases in CDS
    bp_cds_total: Total bases in CDS
    ...
    GC_cds: Percentage of 'G' and 'C' bases in CDS
    cds_topological_entropy: Topological entropy calculated from CDS sequences
    cds_chargaff_pf: Chargaff's parity rule 2 score for CDS
    cds_chargaff_ct: Chargaff's CT score for CDS
    cds_shannon: Shannon entropy score for CDS
    bp_cds_overlap_total: Number of bases overlapping between CDS on two strands
    bp_CDSintron_A: Number of 'A' bases in introns between CDS
    bp_CDSintron_T: Number of 'T' bases in introns between CDS
    bp_CDSintron_C: Number of 'C' bases in introns between CDS
    bp_CDSintron_G: Number of 'G' bases in introns between CDS
    bp_CDSintron_N: Number of 'N' bases in introns between CDS
    bp_CDSintron_total: Total bases in introns between CDS
    fr_CDSintron_A: Frequency of 'A' bases in introns between CDS
    fr_CDSintron_T: Frequency of 'T' bases in introns between CDS
    fr_CDSintron_C: Frequency of 'C' bases in introns between CDS
    fr_CDSintron_G: Frequency of 'G' bases in introns between CDS
    fr_CDSintron_N: Frequency of 'N' bases in introns between CDS
    GC_CDSintron: Percentage of 'G' and 'C' bases in introns between CDS
    CDSintron_topological_entropy: Topological entropy calculated from intron sequences between CDS
    CDSintron_chargaff_pf: Chargaff's parity rule 2 score for introns between CDS
    CDSintron_chargaff_ct: Chargaff's CT score for introns between CDS
    CDSintron_shannon: Shannon entropy score for introns between CDS
    bp_CDSintron_overlap_total: Number of bases overlapping between introns between CDS on two strands
    bp_ncRNA_A: Number of 'A' bases in ncRNA
    bp_ncRNA_T: Number of 'T' bases in ncRNA
    bp_ncRNA_C: Number of 'C' bases in ncRNA
    bp_ncRNA_G: Number of 'G' bases in ncRNA
    bp_ncRNA_N: Number of 'N' bases in ncRNA
    bp_ncRNA_total: Total bases in ncRNA
    ...
    GC_ncRNA: Percentage of 'G' and 'C' bases in ncRNA
    ncRNA_topological_entropy: Topological entropy calculated from ncRNA sequences
    ncRNA_chargaff_pf: Chargaff's parity rule 2 score for ncRNA
    ncRNA_chargaff_ct: Chargaff's CT score for ncRNA
    ncRNA_shannon: Shannon entropy score for ncRNA
    bp_ncRNA_overlap_total: Number of bases overlapping between ncRNA on two strands
    bp_ncintron_A: Number of 'A' bases in introns between ncRNA
    bp_ncintron_T: Number of 'T' bases in introns between ncRNA
    bp_ncintron_C: Number of 'C' bases in introns between ncRNA
    bp_ncintron_G: Number of 'G' bases in introns between ncRNA
    bp_ncintron_N: Number of 'N' bases in introns between ncRNA
    bp_ncintron_total: Total bases in introns between ncRNA
    fr_ncintron_A: Frequency of 'A' bases in introns between ncRNA
    fr_ncintron_T: Frequency of 'T' bases in introns between ncRNA
    fr_ncintron_C: Frequency of 'C' bases in introns between ncRNA
    fr_ncintron_G: Frequency of 'G' bases in introns between ncRNA
    fr_ncintron_N: Frequency of 'N' bases in introns between ncRNA
    GC_ncintron: Percentage of 'G' and 'C' bases in introns between ncRNA
    ncintron_topological_entropy: Topological entropy calculated from intron sequences between ncRNA
    ncintron_chargaff_pf: Chargaff's parity rule 2 score for introns between ncRNA
    ncintron_chargaff_ct: Chargaff's CT score for introns between ncRNA
    ncintron_shannon: Shannon entropy score for introns between ncRNA
    bp_ncintron_overlap_total: Number of bases overlapping between introns between ncRNA on two strands
    bp_tRNA_A: Number of 'A' bases in tRNA
    bp_tRNA_T: Number of 'T' bases in tRNA
    bp_tRNA_C: Number of 'C' bases in tRNA
    bp_tRNA_G: Number of 'G' bases in tRNA
    bp_tRNA_N: Number of 'N' bases in tRNA
    bp_tRNA_total: Total bases in tRNA
    ...
    GC_tRNA: Percentage of 'G' and 'C' bases in tRNA
    tRNA_topological_entropy: Topological entropy calculated from tRNA sequences
    tRNA_chargaff_pf: Chargaff's parity rule 2 score for tRNA
    tRNA_chargaff_ct: Chargaff's CT score for tRNA
    tRNA_shannon: Shannon entropy score for tRNA
    bp_tRNA_overlap_total: Number of bases overlapping between tRNA on two strands
    bp_rRNA_A: Number of 'A' bases in rRNA
    bp_rRNA_T: Number of 'T' bases in rRNA
    bp_rRNA_C: Number of 'C' bases in rRNA
    bp_rRNA_G: Number of 'G' bases in rRNA
    bp_rRNA_N: Number of 'N' bases in rRNA
    bp_rRNA_total: Total bases in rRNA
    ...
    GC_rRNA: Percentage of 'G' and 'C' bases in rRNA
    rRNA_topological_entropy: Topological entropy calculated from rRNA sequences
    rRNA_chargaff_pf: Chargaff's parity rule 2 score for rRNA
    rRNA_chargaff_ct: Chargaff's CT score for rRNA
    rRNA_shannon: Shannon entropy score for rRNA
